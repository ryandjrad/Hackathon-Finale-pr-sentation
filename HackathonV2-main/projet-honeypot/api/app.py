...
def create_threat():
    ...
    try:
        ...
        # Correction du bug de None
        if attacker.total_attacks is None:
            attacker.total_attacks = 0
...
def get_stats():
    ...
    try:
        ...
        timeline = db.session.query(
            func.date_trunc('hour', Threat.timestamp).label('hour'),
            func.count(Threat.id).label('count')
        ).filter(Threat.timestamp >= since)\
         .group_by('hour')\
         .order_by('hour').all()
        ...
        for hour, count in timeline:
            timeline_dict[hour.isoformat()] = count
        ...
        return jsonify({
            ...
            'timeline': timeline_dict  # NOUVEAU
        })
