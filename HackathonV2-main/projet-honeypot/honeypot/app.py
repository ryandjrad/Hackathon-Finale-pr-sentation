...
class AttackLogger:
    ...
    async def log_attack(self, service: str, attacker_ip: str, attacker_port: int, 
        ...
        if not validate_ip(attacker_ip):
            logger.warning(f"Invalid IP address detected: {attacker_ip}")
            return None
...
class SSHHoneypot:
    ...
    async def handle_connection(self, reader, writer):
        ...
        if not addr or not validate_ip(addr[0]):
            logger.warning(f"Invalid connection from {addr}")
            writer.close()
    ...
    async def handle_request(self, request):
        ...
        if path in self.fake_paths:
            ...
            try:
                ...
                if 'cmd=' in post_data or 'exec' in post_data:
